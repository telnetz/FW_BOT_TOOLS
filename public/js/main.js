const wax = new waxjs.WaxJS({
    rpcEndpoint: 'https://wax.eu.eosamsterdam.net',
    tryAutoLogin: true,
...
async function toolsClaim() {
	...
	try {
        ...
        getIndex_Crops.forEach(async element => {
			addTableCrops(element.name,element.times_claimed,sec2time(element.next_availability - Date.now() / 1000));
            if (element.next_availability - Date.now() / 1000 < 0) {
                let config = {
                    actions: [{
                            account: 'farmersworld',
